...
### ############################################################################################################
### ############################################################################################################
SiteName='[COLOR white][B]Dub[/B]Happy.eu[/COLOR]  [v0.1.3]  [Anime]'
SiteTag='dubhappy.eu'
mainSite='http://www.dubhappy.eu/'
iconSite='http://i.imgur.com/P4KrVta.png' #'http://i.imgur.com/0FpysaC.jpg' #'http://i.imgur.com/yY5APdk.png' #'http://www.dubhappy.eu/wp-content/themes/Dubhappy/images/header.jpg'
fanartSite='http://i.imgur.com/yY5APdk.png' #'http://i.imgur.com/0FpysaC.jpg' #'http://www.dubhappy.eu/wp-content/themes/Dubhappy/images/header.jpg'
...
def About(head=''+cFL(SiteName,'blueviolet')+'',m=''):
	...
	if len(m)==0:
		...
		m+=CR+'videofun.me | video44.net | novamov.com | yourupload.com | putlocker.com | mp4upload.com' # | play44.net | vidzur.com'
		m+=CR+CR+'Features:  '
...
def Browse_Episodes(url,page='',content='episodes',view='515'):
	...
	ItemCount=len(matches)
	if ItemCount > 0:
		try: s='<div id="catDesc">\s*\n*\s*<table.*?>\s*\n*\s*<tbody>\s*\n*\s*<tr>\s*\n*\s*<td.*?>\s*\n*\s*<img.*?src="(http://dubhappy.eu/images/.*?\..+?)".*?>\s*\n*\s*</td>\s*\n*\s*<td.*?>'; _img=re.compile(s).findall(htmlA)[0]; _fimg=_img
		except: _img=''+thumbnail; _fimg=''+fanart
		try: s='<h2.*?>Plot Summary</h2>\s*\n*\s*<em.*?>\s*\n*\s*<p>\s*\n*\s*<p>\s*\n*\s*(.*?)\s*\n*\s*</p>'; _plot=re.compile(s).findall(htmlA)[0]
		except: _plot=''
		for _date,_url,_name in matches:
			labs={}; labs['plot']=''+cFL(_plot,colors['12'])
			img=''+thumbnail; fimg=''+fanart ##img=_artIcon; fimg=_artFanart
			img=_img; fimg=_fimg
			_title=''+cFL_(_name,'pink')
			contextLabs={'title':_name,'year':'0000','url':_url,'img':img,'fanart':fimg,'DateAdded':_date}; contextMenuItems=ContextMenu_Episodes(labs=contextLabs)
			pars={'mode':'Hosts','site':site,'section':section,'title':_name,'url':_url,'img':img,'fanart':fimg}
			labs['title']=_title.replace(' Episode ',' [CR]Episode ').replace(' English Dubbed','  '+cFL('[Eng Dub]',colors['6']))+'  '+cFL('['+cFL(_date,colors['10'])+']',colors['11'])
			if ' English Dubbed' in _title: labs['plot']=cFL('Language:  ',colors['11'])+cFL('Eng Dub',colors['6'])+CR+labs['plot']
			labs['plot']=CR+cFL('Date Added:  ',colors['11'])+cFL(_date,colors['10'])+CR+labs['plot']
...
def Browse_List(url,page='',content='tvshows',view='515'):
	...
	if ItemCount > 0:
		for _url,_name in matches:
			...
			labs['title']=_title
			if '-english-dubbed/' in _url.lower(): labs['plot']+=CR+cFL('Language:  ',colors['11']).replace(' English Dubbed','  ')+cFL('Eng Dub',colors['6'])
...
def Browse_Hosts(url,page='',content='list',view='50'):
	...
	set_view(content,view_mode=addst('links-view')); eod()
def Browse_AZ():
	if section=='movies':
		tUrl=mainSite+'alpha-movies/'; scolor=colors['1']
	...
	if section=='series':
		tUrl=mainSite+'alpha-anime/'; scolor=colors['2']
		_addon.add_directory({'mode':'Page','site':site,'section':section,'url':tUrl+'others'},{'title':cFL_('#',scolor)},is_folder=True,fanart=fanartSite,img=iconSite)
		for az in MyAlphabet: _addon.add_directory({'mode':'Page','site':site,'section':section,'url':tUrl+az},{'title':cFL_(az.upper(),scolor)},is_folder=True,fanart=fanartSite,img=iconSite)
	set_view('list',view_mode=addst('default-view')); eod()
def Browse_Genres(url=''):
	if section=='movies': url=mainSite+'anime-movie-genres'; scolor=colors['1']
	if section=='series': url=mainSite+'anime-genres'; scolor=colors['2']
	if url=='': return
	deb('url',url)
	html=nURL(url)
...
def Fav_List(site='',section='',subfav=''):
	...
	for (_name,_year,_img,_fanart,_Country,_Url,_plot,_Genres,_site,_subfav,_section,_ToDoParams,_commonID,_commonID2) in favs:
		...
		contextMenuItems=ContextMenu_Favorites(contextLabs) #contextMenuItems=[]
...
def SubMenu(): #(site,section=''):
	if section=='movies':
		scolor=colors['1']
		_addon.add_directory({'mode':'List','site':site,'section':section,'url':mainSite+'anime-movies'},{'title':cFL_('Anime Movies',colors['1'])},is_folder=True,fanart=fanartSite,img=iconSite)
	...
	if section=='series':
		scolor=colors['6'] #['2']
		_addon.add_directory({'mode':'List','site':site,'section':section,'url':mainSite+'anime-list'},{'title':cFL_('Anime List',colors['6'])},is_folder=True,fanart=fanartSite,img=iconSite)
...
def SectionMenu():
	...
	_addon.add_directory({'mode':'SubMenu','site':site,'section':'series'},{'title':cFL_('Anime Series',colors['6'])},is_folder=True,fanart=fanartSite,img=iconSite)
