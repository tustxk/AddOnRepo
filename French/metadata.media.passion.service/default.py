...
# addon constants
ServiceId  = "metadata.media.passion.service"
...
class Main:
    ...
    def update_credits( self, updatetime ):
        ...
        if xbmc.getCondVisibility( "System.HasAlarm(UpdateCreditsMediaPassion)" ):
            xbmc.executebuiltin( "XBMC.CancelAlarm(UpdateCreditsMediaPassion,true)" )

        if updatetime > 0:
            # setup auto update credits Cine-Passion
            command = "%s,%i,true" % ( RunService, updatetime * 60 )
            xbmc.executebuiltin( "XBMC.AlarmClock(UpdateCreditsMediaPassion,%s)" % command )
