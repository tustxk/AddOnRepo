...
from cinetrafic_common import *
from cinetrafic_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def About(head=''+cFL(SiteName,'blueviolet')+'',m=''):
	...
	if len(m)==0:
		...
		m+=CR+'* [cinetrafic.fr], [youtube.com]'
	...
	if iC > 0:
		...
		for (url1,domain) in html2p1:
			...
			if phtml2p==url1: next
					...
					if (domain.lower() in ['cinetrafic.fr','www.cinetrafic.fr']):
							quality=addst('quality-cinetrafic.fr','HD'); 
							if quality.upper()=='HD': qB=''
							elif quality.upper()=='SD': qB='_sd'
							else: qB=''
							try: vID=re.compile('onclick="report_bug\(\'video_(\d+)\'\)">').findall(html)[0]; deb(domain+' vID',vID); 
							except: vID='0'
							qUrl='http://video.cinetrafic.fr/videos/'+str(vID)+qB+'.mp4'; 
							img='http://video.cinetrafic.fr/videos/'+str(vID)+'.png'; 
							cMI=[]; labs={}; pars={'site':site,'mode':'PlayURL','url':qUrl,'title':title,'studio':''+name+' - ['+domain+']'+' [UrlResolver]','img':img,'fimg':fimg,'wwT':wwT,'imdb_id':imdb_id }
...
def ListShows(Url):
	...
	deb('Url',Url); html=messupText(nolines(nURL(Url)),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	try: CurPageNum=int(Url.split('/')[-1])
	except: CurPageNum=1
	s ='<div class="film_\D+_small">\s*<a href="(.+?)" class="classic_link">\s*<strong>\s*(.+?)\s*</strong>\s*</a>\s*'; 
	s+='<br/>\s*<span class="text_\D+ droid\d*">\s*\((\d+),\s*(.+?)\)\s*</span>\s*'; 
	s+='</div>\s*</div>\s*<div class="item_film_\D+_small">\s*'; 
	s+='.*?<a.*?>\s*<img.*?src="(.+?)".*?>'; 
	#s ='<div class="postcontent">\s*'; 
	#s+='<a href=".+?">\s*\s*<img class=\'ep_thumb\' src=\'(.+?)\' alt=\'.*?\'\s*[/]*>\s*</a>\s*'; 
	#s+='<div class="post_content">\s*'; 
	#s+='<h2 class="postitle">\s*<a href="(.+?)">\s*(.+?)\s*</a></h2>\s*'; 
	#s+='<p>\s*(.*?)\s*</p>\s*'; 
	...
	if len(matches) > 0:
		...
		if (CurPageNum > 1) and (Url[0-len(str(CurPageNum)):len(Url)]==str(CurPageNum)):
			PrEVIOUS=Url[0:( len(Url) - len(str(CurPageNum)) )]+str(CurPageNum-1)
			_addon.add_directory({'mode':'ListShows','url':PrEVIOUS,'site':site,'section':section},{'title':cFL('<< Precedent','green')+'  '+PrEVIOUS.replace(mainSite,'').replace(mainSite2,'').replace('/category','')},is_folder=True,fanart=fanartSite,img=psgn('img_prev'))
		if enMeta==True:
		...
		#for (img,url,name,sitePlot) in matches:
		for (url,name,year,sitePlot,img) in matches:
			name=''+name+' ('+str(year)+')'; 
			name1=''+name; 
		...
		try: LaSTno=re.compile('<a href="#" onclick="linker.go\(\'\d*\!\|\d*\!\D*\!\D*-\D*!\D*-\!\|\D*\', \d*\);return false"><div class=\'page_off droid\d*\'>(\d+)</div></a></div></div></div>').findall(html)[0]; deb('LaSTno',LaSTno); 
		except: LaSTno=1
		if (CurPageNum < LaSTno) and (Url[0-len(str(CurPageNum)):len(Url)]==str(CurPageNum)):
			NeXT=Url[0:( len(Url) - len(str(CurPageNum)) )]+str(CurPageNum+1)
			_addon.add_directory({'mode':'ListShows','url':NeXT,'site':site,'section':section},{'title':cFL('>> Suivant','green')+'  '+NeXT.replace(mainSite,'').replace(mainSite2,'').replace('/category','')},is_folder=True,fanart=fanartSite,img=psgn('img_next'))
		if (LaSTno > 1) and (Url[0-len(str(CurPageNum)):len(Url)]==str(CurPageNum)):
			LaST=Url[0:( len(Url) - len(str(CurPageNum)) )]+str(LaSTno)
			_addon.add_directory({'mode':'ListShows','url':LaST,'site':site,'section':section},{'title':cFL('>> >> Dernier','green')+'  '+LaST.replace(mainSite,'').replace(mainSite2,'').replace('/category','')},is_folder=True,fanart=fanartSite,img=psgn('img_last'))
...
def MenuGenre():
	for aL,azRo,azEn in MyGenres: 
		...
		_addon.add_directory({'mode':'ListShows','url':'/'+aL.lower().replace(' ','-')+'/1','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
...
def SectionMenu():
	...
	#_addon.add_directory({'mode':'GetMedia','site':site,'url':'/film/38787/x-men-days-of-future-past','title':'Testing'},{'title':AFColoring('Testing')},is_folder=True,fanart=fanartSite,img=iconSite)
	#_addon.add_directory({'mode':'GetMedia','site':site,'url':'/film/34031/le-hobbit-un-voyage-inattendu','title':'Testing'},{'title':AFColoring('Testing 2')},is_folder=True,fanart=fanartSite,img=iconSite)
	...
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/film-a-voir/1'},{'title':RoAFColoring('Films','Movies to view')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/super-film/1'},{'title':RoAFColoring('Super film','Super Movies')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/top-film/1'},{'title':RoAFColoring('Top film','Top Movies')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/dernier-bon-film/1'},{'title':RoAFColoring('Bon film','Good Movies')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/film-recent/1'},{'title':RoAFColoring('Film recent','Recent Movies')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/nouveau-film/1'},{'title':RoAFColoring('Nouveau film','New Movies')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/sortie-cinema/1'},{'title':RoAFColoring('Sortie Cinema','Cinema Releases')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/meilleures-series/1'},{'title':RoAFColoring('Series','Series')},is_folder=True,fanart=fanartSite,img=psgn('all')) #iconSite)
	...
	## #_addon.add_directory({'mode':'Search','site':site,'url':'/recherche/film/'},{'title':RoAFColoring('Rechercher','Search')},is_folder=True,fanart=fanartSite,img=psgn('search'))
	## #_addon.add_directory({'mode':'Search','site':site,'url':'/recherche/video-extrait/'},{'title':RoAFColoring('Rechercher','Search')},is_folder=True,fanart=fanartSite,img=psgn('search'))
