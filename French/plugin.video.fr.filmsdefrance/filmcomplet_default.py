...
from filmcomplet_common import *
from filmcomplet_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def ListShows(Url):
	...
	#s+='<img style="height:188px;width:128px;" src="(.+?)" alt=".+?" /></a></h2>\s*'; 
	#s+='<h2 class="entry-title">\s*<a href=".+?" title="\s*.+?" rel="bookmark">.+?</a></h2>\s*'; 
	#s+='</div><!-- #post-## --'; 
	### img,url,title
...
def MenuGenre():
	for aL,azRo,azEn in sorted(MyGenres,key=lambda item: (item[1]),reverse=False): 
		...
		_addon.add_directory({'mode':'ListShows','url':'/category/'+aL.lower().replace(' ','-')+'/','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
...
def SectionMenu():
	...
	#_addon.add_directory({'mode':'GetMedia','site':site,'url':'/the-river-why/','title':'Testing'},{'title':AFColoring('Testing')},is_folder=True,fanart=fanartSite,img=iconSite)
	#_addon.add_directory({'mode':'GetMedia','site':site,'url':'/38-temoins/','title':'Testing'},{'title':AFColoring('Testing 2')},is_folder=True,fanart=fanartSite,img=iconSite)
