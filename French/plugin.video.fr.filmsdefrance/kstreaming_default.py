...
from kstreaming_common import *
from kstreaming_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def GetMedia(Url,title,imdb_id,img,fimg,stitle,etitle,enumber,snumber,enumber2,wwT=''):
	...
	html=html.replace('</span></a> <a href="','</span></a> \n<a href="')
	#s ='<a href="('+Url+'[/]*\d+)"><span>(.+?)</span></a>'; 
	s ='<a href="('+Url+'.*?)"><span>(.+?)</span></a>'; 
	try: UrlS=re.compile(s).findall(html); debob(UrlS); 
	except: UrlS=[]
	if '/serie' in Url: UrlS.append((Url1,'Part 1'))
	...
	if iC > 0:
		...
		for (url1,domain,EpTag) in html2p1:
			...
			if '/serie' in Url: domain=EpTag+' - '+domain
...
def ListShowsH(Url,html):
	...
	s+='\s*(?:<span class=".*?"></span>)?<img src="(.+?)" alt=".*?" height="\d+px" width="\d+px" /></a>'; 
	...
	if len(matches) > 0:
		...
		if "' class='last'>Derni" in html:
			try: LaST=html.split("' class='last'>Derni")[0].split("<a href='")[-1]; #re.compile("<a href='("+mainSite+"/.+?/)' class='last'>Last ").findall(html)[0]; 
			except: LaST=Url; 
...
def SectionMenu():
	...
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/category/serie/'},{'title':RoAFColoring('Nouveautes Series','Latest TV Shows')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
