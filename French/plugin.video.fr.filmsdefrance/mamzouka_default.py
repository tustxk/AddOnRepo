...
from mamzouka_common import *
from mamzouka_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def About(head=''+cFL(SiteName,'blueviolet')+'',m=''):
	...
	if len(m)==0:
		...
		m+=CR+'* Search for Movies'
		m+=CR+'* Browse TV Shows, Host Links (with limitations, lacks episode numbers)'
		m+=CR+'* Search for TV Series'
		##m+=CR+'* Download Videos with UrlResolver'
		#m+=CR+'* '
		m+=CR+CR+'Notes:  '
		#m+=CR+'* Thanks to: ..., Eleazar, and TheHighway.'
		#m+=CR+'* '+ps('ReferalMsg')
		m+=CR+'* TV Section is currently a work in progress.  Due to the way things are done on the site, the series section had to be limited.'
		m+=CR+'* TV section est actuellement en cours. En raison de la facon dont les choses sont faites sur le site, la section de la serie devait etre limitee.'
		m+=CR+''
		m+=CR+''
		m+=CR+''
	try: DoA('Back'); 
...
def GetMedia(Url,title,imdb_id,img,fimg,stitle,etitle,enumber,snumber,enumber2,wwT=''):
	...
	if len(Url)==0: return
	if ('http://' not in Url) and ('https://' not in Url) and (mainSite not in Url) and (mainSite2 not in Url): Url=mainSite+Url; 
	###
	...
	###
	html2p1=[]; UrlS=[]; Url1=''+Url; 
	#if (mainSite not in Url1) and (mainSite2 not in Url1): Url1=mainSite+Url1; 
	deb('imdb_id',imdb_id); deb('title',title); deb('Url',Url1); html=messupText(nolines(nURL(Url1)),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	if 'http://series.mamzouka.com/' in Url:
		s ="<li><a href='("+Url+"/\d+)'>\s*(Episode.+?)\s*</a></li"; 
		UrlS.append((Url1,'Episode 1'))
	else:
		s ='<a href="('+Url+'/\d+)"><span>(.+?)</span></a>'; 
		try: UrlS=re.compile(s).findall(html); debob(UrlS); 
		except: UrlS=[]
		UrlS.append((Url1,''))
	#if 'http://series.mamzouka.com/' in Url:
	if len(UrlS) > 0:
		for (Urls,UrlSep) in UrlS:
			try: 
				xbmc.sleep(30)
				htmlT=messupText(nolines(nURL(Urls)),True,True); 
				#htmlT=htmlT.replace(,UrlSep).replace(UrlSep,UrlSep)
				#html+=htmlT; deb('length of html',str(len(html))); #debob(html); 
				htmlT=htmlT.replace('</iframe>','</iframe\n>').replace('</IFRAME>','</IFRAME\n>'); 
			except: pass
			s2=['<iframe .*?src="(\D*[\:]*//(.+?)/.+?)".*?>\s*</iframe',"<iframe .*?src='(\D*[\:]*//(.+?)/.+?)'.*?>\s*</iframe","<IFRAME .*?SRC='(\D*[\:]*//(.+?)/.+?)'.*?>\s*</IFRAME",'<IFRAME .*?SRC="(\D*[\:]*//(.+?)/.+?)".*?>\s*</IFRAME']; 
			for s in s2:
				try: html2p1b=re.compile(s).findall(htmlT); #,re.IGNORECASE
				except: html2p1b=[]
				for p1b,p1c in html2p1b: html2p1.append((p1b,p1c,UrlSep))
	#s2=['<iframe .*?src="(\D*[\:]*//(.+?)/.+?)".*?>\s*</iframe>',"<iframe .*?src='(\D*[\:]*//(.+?)/.+?)'.*?>\s*</iframe>","<IFRAME .*?SRC='(\D*[\:]*//(.+?)/.+?)'.*?>\s*</IFRAME>",'<IFRAME .*?SRC="(\D*[\:]*//(.+?)/.+?)".*?>\s*</IFRAME>']; 
	#for s in s2:
	#	try: html2p1b=re.compile(s).findall(html); #,re.IGNORECASE
	#	except: html2p1b=[]
	#	for p1b in html2p1b: html2p1.append(p1b)
	iC=len(html2p1); debob(html2p1); 
	if iC > 0:
		if not 'http://series.mamzouka.com/' in Url:
			aSortMeth(xbmcplugin.SORT_METHOD_TITLE); 
		phtml2p=""; CentA=''; #html2p1=sorted(html2p1); 
		debob(html2p1); 
		for (url1,domain,EpTag) in html2p1:
			name=title; domain=domain.replace('www.',''); #url1=url1.replace('&#038;','&'); 
			if 'series-tv' in Url: domain=EpTag+' - '+domain
			if phtml2p==url1: next
					...
					if domain in ['vidto.me']:
						try:
							if '/embed-' in url1: url1=url1.replace('embed-','')
							if '.html' in url1: url1=url1.replace('.html','')
...
def ListShowsH(Url,html):
	...
	#deb('Url',Url); html=messupText(nolines(nURL(Url)),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	s ='<div class="movieposter" title=".+?">\s*<a href="(.+?)"><img src="(.+?)" width="\d+" height="\d+" alt=".*?" title=".*?" /></a>\s*<div class="shortname">(.+?)(?:\s+Streaming)?</div>\s*<div class=".*?">(.*?)</div>'; 
	if 'http://series.mamzouka.com/' in Url:
		s ='<div class="imageholder">\s*<a href="(.+?)" class=\'.*?\' title=\'.*?\'\s*>\s*<span class="hover-image">\s*<small>\s*</small>\s*</span>'; 
		s+="\s*<img src='(.+?)' alt=''\s*/>\s*</a>\s*</div>"; 
		s+='\s*<div class="description.*?">\s*<h2 class="custom-font"><a href=".+?">\s*(.+?)\s*</a></h2>'; 
		s+='\s*<span class="date">\s*.+?\s*</span>'; 
		s+='\s*<span class="comment">\s*<a href=".+?" title=".+?">.+?</a>\s*</span>'; 
		s+='\s*<p>\s*(.+?)\s*</p>'; 
	##
	### img,url,title
	...
	### url,img,name,plot
	html=html.replace('<div class="leftpane">','\n<div  class="leftpane">'); 
	html=html.replace('<li class="clearfix">','<li \nclass="clearfix">'); 
	...
	if len(matches) > 0:
		...
		enMeta=False; 
		if ' Previous</a> <a href="' in html:
			try: PrEVIOUS= html.split(' Previous</a> <a href="')[0].split('<a href="')[-1].split('">')[0]; #re.compile("<a href='(.+?)' class='previouspostslink'>").findall(html)[0]; 
			except: PrEVIOUS=Url; 
		...
		for (url,img,name,Quality) in matches: #for (url,img,name) in matches:
			sitePlot=''+Quality; #sitePlot=''; 
			year=''; 
			...
			#else: TyP='movie'
			TyP='movie'
			if 'http://series.mamzouka.com/' in Url: TyP='tv'
			...
			if not 'http://series.mamzouka.com/' in Url:
				labs[u'title']+=cFL(' ['+cFL(Quality,colorB)+']',colorA)
		...
		if '>Next ' in html:
			try: NeXT=html.split('">Next ')[0].split('<a href="')[-1]; #re.compile("</span><a href='("+mainSite+"/.+?/)' class='nextpostslink'>").findall(html)[0]; 
			except: NeXT=Url; 
		...
		if '">Next ' in html:
			try: LaST=html.split('">Next ')[0].split('<a href="')[-2].split('">')[0]; #re.compile("<a href='("+mainSite+"/.+?/)' class='last'>Last ").findall(html)[0]; 
			except: LaST=Url; 
...
def DoSearch(title='',Url='/'):
	...
	deb('Url',Url); html=messupText(nolines(nURL(Url,method='post',form_data={'story':title,'page':'','subaction':'search','do':'search'})),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	ListShowsH(Url,html)
	##
...
def MenuYear(Url=''):
	for az in MyYears: 
		i='' #psgn(az.lower())
		if len(i)==0: i=iconSite
		if 'http://series.mamzouka.com' in Url: Url+='/category/'
		else: Url+='/date-de-sortie-'
		_addon.add_directory({'mode':'ListShows','url':Url+''+az.lower().replace(' ','-')+'/','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
	set_view('list',view_mode=addst('list-view')); eod()
...
def SectionMenu():
	...
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/'},{'title':RoAFColoring('Nouveautes Films','Latest Movies')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'http://series.mamzouka.com/2014/'},{'title':RoAFColoring('2014 Serie','2014 Series')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	_addon.add_directory({'mode':'MenuAZ','site':site,'url':'/cat'},{'title':RoAFColoring('Alphabetique','A-Z Movies')},is_folder=True,fanart=fanartSite,img=psgn('a-z')) #iconSite)
	...
	_addon.add_directory({'mode':'MenuYear','site':site,'url':''},{'title':RoAFColoring('Annees','Years')},is_folder=True,fanart=fanartSite,img=psgn('year'))
	_addon.add_directory({'mode':'Search','site':site,'url':'/'},{'title':RoAFColoring('Rechercher','Search')},is_folder=True,fanart=fanartSite,img=psgn('search'))
	...
	_addon.add_directory({'mode':'FavoritesList','site':site,'section':section,'subfav':'2'},{'title':cFL(ps('WhatRFavsCalled'),colorA)+cFL('Seriale - TV Shows',colorB)+oStar},fanart=fanartSite,img=psgn('favorites 2'))
	
	_addon.add_directory({'mode':'MenuYear','site':site,'url':'http://series.mamzouka.com'},{'title':RoAFColoring("Serie par l'annee",'Series by Year')+oStar},is_folder=True,fanart=fanartSite,img=psgn('year'))
	_addon.add_directory({'mode':'SearchS','site':site,'url':'http://series.mamzouka.com/?cat=all&s='},{'title':RoAFColoring('Recherche de serie','Search Series')+oStar},is_folder=True,fanart=fanartSite,img=psgn('search'))
...
def mode_subcheck(mode='',site='',section='',url=''):
	...
	elif (mode=='SearchS'):				DoSearchS(addpr('title',''),url)
	...
	elif (mode=='MenuYear'): 		MenuYear(url)
	elif (mode=='FavoritesList'): Fav_List(site=site,section=section,subfav=addpr('subfav',''))
