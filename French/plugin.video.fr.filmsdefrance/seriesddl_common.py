...
def ParseDescription(plot): ## Cleans up the dumb number stuff thats ugly.
	...
	if ("&laquo;" in plot):  plot=plot.replace('&laquo;' ,"<<")
	if ("&raquo;" in plot):  plot=plot.replace('&raquo;' ,">>")
	if ("&lt;" in plot):  plot=plot.replace('&lt;' ,"<")
	if ("&gt;" in plot):  plot=plot.replace('&gt;' ,">")
	if ("&#8211;" in plot): plot=plot.replace("&#8211;","-") #unknown
	if ("&#8230;" in plot): plot=plot.replace("&#8211;","...") #unknown
	if ("&rarr;" in plot):  plot=plot.replace('&rarr;' ,"->")
...
def PlayProtectedURL(url):
	...
	html=nURL('http://www.protectup.com/linkid.php',method='post',form_data=data); #html=net.http_POST(url, data).content
	#debob(html); 
	s='<a href=(\D+://.+?)\s+target=_blank>'
	url=re.compile(s).findall(html)[0]; deb('url',url); 
	
	try: _addon.resolve_url(urlF) #url)
	except: t=''
