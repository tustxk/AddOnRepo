...
from sokrostream_common import *
from sokrostream_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def About(head=''+cFL(SiteName,'blueviolet')+'',m=''):
	...
	if len(m)==0:
		...
		m+=CR+'* [hqq.tv]'
		m+=CR+CR+'Features:  '
		m+=CR+'* Browse Movies, TV Shows, Host Links'
					...
					if (tfalse(addst('internal-vk.com','false'))==True) and (domain.lower() in ['vk.com','vk.me']):
						...
						if len(rLink)==0:
							...
							for rW,rQ,rL in rLink:
								if (rW.lower() in ['url','cache']) and (not pLink==rL):
									quality=rW+' '+rQ; 
...
def ListShowsH(Url,html):
	...
	#deb('Url',Url); html=messupText(nolines(nURL(Url)),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	html=html.replace('<div class="moviefilm">','<\ndiv class="moviefilm">').replace('<\n','\n<'); 
	s ='<div class="moviefilm">\s*<a href=".+?">'; 
	s+='\s*(?:<span class=".*?"></span>)?<img src="(.+?)" alt=".*?" height="\d+\D*" width="\d+\D*" /></a>'; 
	s+='\s*<div class="movief"><a href="(.+?)">\s*(.+?)\s*</a></div>'; 
	s+='\s*<div class="movies">(?:<small>\s*.*?\s*</small>)?</div>\s*</div'; 
	##
	### img,url,title
	...
	if len(matches) > 0:
		...
		for (img,url,name) in matches: #for (url,img,name) in matches:
			sitePlot=''; year=''; 
...
def MenuYear():
	for az in MyYears: 
		i='' #psgn(az.lower())
		if len(i)==0: i=iconSite
		_addon.add_directory({'mode':'ListShows','url':'/annees/'+az.lower().replace(' ','-')+'/','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
	set_view('list',view_mode=addst('list-view')); eod()
def SectionMenu():
	...
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/series-tv'},{'title':RoAFColoring('Nouveautes Series','Latest TV Shows')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	...
	if (len(addst(site+"_LastShowListedURL")) > 0): 
		...
		#title=AFColoring(addst(site+"_LastShowListedNAME"))+CR+RoAFColoring('[Film Derniere]','[Last Movie]').replace(colorB,colorA); #cFL(' [Last Show]',colorA); 
