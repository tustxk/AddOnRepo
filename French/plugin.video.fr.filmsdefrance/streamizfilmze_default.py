...
from streamizfilmze_common import *
from streamizfilmze_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def GetMedia(Url,title,imdb_id,img,fimg,stitle,etitle,enumber,snumber,enumber2,wwT=''):
	...
	if 'series-tv' in Url: UrlS.append((Url1,'Ep 01'))
	else: UrlS.append((Url1,''))
	if len(UrlS) > 0:
...
def ListShowsH(Url,html):
	...
	#deb('Url',Url); html=messupText(nolines(nURL(Url)),True,True); deb('length of html',str(len(html))); #debob(html); 
	if len(html)==0: return
	s ='<a href="(.+?)"><img src="(.+?)" width="127" height="170" ALIGN="\D+" border="1"'; 
	s+=" alt='.*?' title='(.+?)'></a>"; 
	##
	### img,url,title
	...
	if len(matches) > 0:
		...
		enMeta=False; 
		if '"><img src="/templates/darkspace/images/back.png" alt="en arriere" border="0"/></a>' in html:
			try: PrEVIOUS= html.split('"><img src="/templates/darkspace/images/back.png" alt="en arriere" border="0"/></a>')[0].split('href="')[-1]; #re.compile("<a href='(.+?)' class='previouspostslink'>").findall(html)[0]; 
			except: PrEVIOUS=Url; 
		...
		for (url,img,name) in matches: #for (url,img,name) in matches:
			sitePlot=''; year=''; 
		...
		if '"><img src="/templates/darkspace/images/forward.png" alt="suivant" border="0"/></a>' in html:
			try: NeXT=html.split('"><img src="/templates/darkspace/images/forward.png" alt="suivant" border="0"/></a>')[0].split('href="')[-1]; #re.compile("</span><a href='("+mainSite+"/.+?/)' class='nextpostslink'>").findall(html)[0]; 
			except: NeXT=Url; 
...
def DoSearch(title='',Url='/search/results.php'):
	if len(Url)==0: return
...
def MenuYear():
	for az in MyYears: 
		i='' #psgn(az.lower())
		if len(i)==0: i=iconSite
		_addon.add_directory({'mode':'ListShows','url':'/'+az.lower().replace(' ','-')+'/','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
	set_view('list',view_mode=addst('list-view')); eod()
...
def SectionMenu():
	...
	#_addon.add_directory({'mode':'GetMedia','site':site,'url':'/film-thriller/6555-non-stop-streaming.html','title':'Testing'},{'title':AFColoring('Testing')},is_folder=True,fanart=fanartSite,img=iconSite)
	...
	#_addon.add_directory({'mode':'ListShows','site':site,'url':'/Films_de_la_semaine.php'},{'title':RoAFColoring('Ajout de la semaine','Latest this week')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	...
	#_addon.add_directory({'mode':'Search','site':site,'url':'/search/results.php'},{'title':RoAFColoring('Rechercher','Search')},is_folder=True,fanart=fanartSite,img=psgn('search'))
