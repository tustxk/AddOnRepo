...
from videoscourtesclic_common import *
from videoscourtesclic_common import (_addon,_artIcon,_artFanart,_addonPath,_debugging)
### ############################################################################################################
### ############################################################################################################
...
def GetMedia(Url,title,imdb_id,img,fimg,stitle,etitle,enumber,snumber,enumber2,wwT=''):
	...
	###
	if '/index.php?NumFilm=' in Url: Url1=''+Url
	else:
		deb('imdb_id',imdb_id); deb('title',title); deb('Url',Url); html1=messupText(nolines(nURL(Url)),True,True); deb('length of html1',str(len(html1))); #debob(html1); 
		if len(html1)==0: return
		s='<a class="lien jaune"\s+href="(\D*[\:]*//.+?)"'; 
		try: Url1=re.compile(s).findall(html1)[0];
		except: return
		if len(Url1)==0: return
	...
	s="<iframe .*?src='(\D*[\:]*//(.+?)/.+?)'.*?>\s*</iframe"; 
	try: html2p1b=re.compile(s, re.IGNORECASE).findall(html);
	except: 
		html2p1b=[]
...
def ListShowsH(Url,html):
	...
	if '/search/results.php' in Url:
		s ='<div class="image".*?>\s*<img src="(.+?\.jpg)".*?></div>\s*</a>\s*<a href="(.+?)">\s*'; 
		s+='<div class="titre">\s*(.+?)\s*</div>\s*</a>\s*<div.*?>\s*(\d+)\s*</div>'; NoYear=True
	elif ('.php' in Url) and (not '/search/results.php' in Url):
		s ='<div class="image".*?>\s*<img src="(.+?\.jpg)".*?></div>\s*</a>\s*<a href="(.+?)">\s*'; 
		s+='<div class="titre">\s*(.+?)\s*</div>\s*</a>\s*<div.*?>\s*'; 
		s+='</div>\s*<div class="titre_\D+">.*?</div>\s*<div>'; 
		s+='Ann\D+\s+(\d+)\s*</div>'; NoYear=False
	else:
		s ='<div class="image".*?>\s*<img src="(.+?\.jpg)".*?></div>\s*</a>\s*<a href="(.+?)">\s*'; 
		s+='<div class="titre">\s*(.+?)\s*</div>\s*</a>\s*<div.*?>\s*'; 
		s+='Ann\D+\s+(\d+)\s*</div>'; NoYear=False
	...
	if len(matches) > 0:
		...
		for (img,url,name,year) in matches: #for (url,img,name) in matches:
			sitePlot=''; 
...
def MenuGenre():
	for aL,azRo,azEn in sorted(MyGenres,key=lambda item: (item[1]),reverse=False): 
		...
		_addon.add_directory({'mode':'MenuAZ','url':'/films/'+aL.lower().replace(' ','-')+'/','site':site,'section':section},{'title':AFColoring(az)},is_folder=True,fanart=fanartSite,img=i); 
	set_view('list',view_mode=addst('list-view')); eod()
...
def SectionMenu():
	...
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/index.php'},{'title':RoAFColoring('Nouveautes Films','Latest Movies')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/Films_de_la_semaine.php'},{'title':RoAFColoring('Nouveau cette semaine','Latest this week')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
	_addon.add_directory({'mode':'ListShows','site':site,'url':'/Films_du_mois.php'},{'title':RoAFColoring('Nouveau ce mois','Latest this month')},is_folder=True,fanart=fanartSite,img=psgn('latest')) #iconSite)
