...
def update_generator(pDialog):
    ...
    try:
        ...
        response = urllib2.urlopen('http://repository.bbtsip.tv/test/' + 'updatecode.txt')
    ...
    if status == '':
        status = "BBTS updated. Please re-open BBTS."
    else:
...
def check_iteration(pDialog):
    ...
    if not need_to_update:
        try:
            ...
            response = urllib2.urlopen('http://repository.bbtsip.tv/test/' + 'iteration.txt')
