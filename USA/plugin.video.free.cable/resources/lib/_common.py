...
site_descriptions= {'abc': "ABC Television Network provides broadcast programming to more than 220 affiliated stations across the U.S. The Network encompasses ABC News, which is responsible for news programming on television and other digital platforms; ABC Entertainment Group, a partnership between ABC Studios and ABC Entertainment responsible for the networkâ€™s primetime and late-night entertainment programming; ABC Daytime, producer of the networkâ€™s successful cache of daytime programming; as well as ABC Kids, the Networkâ€™s childrenâ€™s programming platform. ABCâ€™s multiplatform business initiative includes the Interactive Emmy Award-winning broadband player on ABC.com.",
                    ...
                    'abcnews': "ABC News is responsible for all of the ABC Television Networkâ€™s news programming on a variety of platforms: TV, radio and the Internet.",
...
settings['enablestrictnames'] = addoncompat.get_setting('strict_names')

def get_series_id(seriesdata,seriesname):
    shows = BeautifulStoneSoup(seriesdata, convertEntities=BeautifulStoneSoup.XML_ENTITIES).findAll('series')
    names = list(BeautifulStoneSoup(seriesdata, convertEntities=BeautifulStoneSoup.XML_ENTITIES).findAll('seriesname'))
    if len(names) > 1:
    ...
    if not infoLabels:
        ...
        if actors<>None:
            ...
            if actors[0] <> '':
                infoLabels['cast']=actors
        if genres<>None: infoLabels['genre']=genres.encode('utf-8', 'ignore').replace('|',',').strip(',')
        #if network<>None: infoLabels['studio']=network.decode('utf-8').encode('utf-8', 'ignore')
        if network<>None: infoLabels['studio']=network.decode('utf-8').encode('utf-8', 'ignore')
        if runtime<>None: infoLabels['duration']=runtime
        if rating<>None: infoLabels['rating']=rating
    name = series_title
...
def getURL( url , values = None ,proxy = False, referer=False,forwardHeader=False):
    try:
        if proxy == True:
...
def getRedirect( url , values = None ,proxy = False, referer=False,forwardHeader=False):
    try:
        if proxy == True:
            us_proxy = 'http://' + addoncompat.get_setting('us_proxy') + ':' + addoncompat.get_setting('us_proxy_port')
            proxy_handler = urllib2.ProxyHandler({'http':us_proxy})
