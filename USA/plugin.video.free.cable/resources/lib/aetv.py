...
def show_cats(url=common.args.url):
    #print "showcats",url,len(url.split('/'))
    #make sure we are on video page not main site
    if len(url.split('/'))<6:
        url=url+'/video'
...
def showsubThePlatform(url=common.args.url,tree=False):
    ...
    for video in videos:
        ...
        infoLabels['Title'] = video_details.find('p',attrs={'class':'video_details-title'}).string.encode('utf-8').replace(' / ','/') #clean title
            ...
            if p.find('span'):
                ...
                if 'Premiere Date: ' in p.string:
                    ...
                    try:
                        ...
                        if common.settings['enablestrictnames'] == 'false':
...
def playThePlatform():
    ...
    #print "data",data #slices debug
    #mrss = urllib.unquote_plus(base64.b64decode(re.compile('{ mrss: "(.+?)",').findall(data)[0]))
    smil_url=re.compile('releaseURL: +"(.*?)"').findall(data)[0]
    #print releaseUrl
    ...
    sig = sign_url(smil_url) #switch to hls
    smil_url += '&sig='+sig
    data = common.getURL(smil_url.replace('switch=hds&',''),proxy=proxy)
    print data
