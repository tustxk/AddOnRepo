...
def rootlist(db=False):
    data = common.getURL(BASE_URL,forwardHeader=True)
    #print "d",data
    tree=BeautifulSoup(data, convertEntities=BeautifulSoup.HTML_ENTITIES)
    shows=tree.findAll('div',attrs={'class':'filter-content-more'})[3].findAll('li')
    db_shows = []
    for show in shows:
        name = show.find('span',attrs={'class':'filter-name'}).string.replace('&','and')
        url = show['data-value']        
        if db==True:
...
def episodes():
    ...
    for episode in episodes:
        ...
        duration=episode.find('span',attrs={'class':'duration'}).string.replace('Duration:','')
        print "d",duration
