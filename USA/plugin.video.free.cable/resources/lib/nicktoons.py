...
def episodes():
    ...
    for episode in episodes:
        ...
        name = h4link.string.replace(', pt.',' Part')
...
def playuri(uri = common.args.url,referer='http://nicktoons.nick.com'):
    ...
    swfUrl = common.getRedirect(mtvn,referer=referer,forwardHeader=True)
    configurl = urllib.unquote_plus(swfUrl.split('CONFIG_URL=')[1].split('&')[0]).strip()
    configxml = common.getURL(configurl,referer=mtvn,forwardHeader=True)
    ...
    mrssxml = common.getURL(mrssurl,forwardHeader=True)
    ...
    for segment in segmenturls:
        surl = segment['url']
        videos = common.getURL(surl,forwardHeader=True)
...
def playvideo(url = common.args.url):
    data=common.getURL(url,forwardHeader=True)
    try:
