...
class Faststream(HostResolver, CustomSettings):
    ...
    def Resolve(self, url):
        ...
        try:
            ...
            if items:
                secs=re.search(r'countdown_str\"\>Wait\s+\<span\sid=\".*?\"\>(\d+)\<',html)
                if secs:
                    common.addon.show_countdown(int(str(secs.group(1))), title=self.name.title(), text='please wait')
                    return re.search(r'file\:\s+"(.*?)\"',net.http_POST(url,postData).content,re.I).group(1)
