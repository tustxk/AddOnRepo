...
class g2g(MovieIndexer,TVShowIndexer,MovieSource,TVShowSource):
    ...
    def Resolve(self, url):
        ...
        if 'ggplayer' in content:
            new_url = re.compile('<iframe id="ggplayer" src="(.+?)"').findall(content)[0]#<iframe id="ggplayer" src="http://g2g.fm/lahimlnhgpdhm/htghl/hercules.php"
        else:
            headers={'Host':'g2g.fm', 'Referer': url , 'User-Agent':'Mozilla/5.0 (Windows NT 6.3; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/35.0.1916.153 Safari/537.36'}
            data=item_url.split('=')[1]
            form_data={ 'g2gmov': data }
        ...
        if 'fmt_stream_map' in html:
            ...
            common.addon.show_small_popup('[B][COLOR blue]I[/B][/COLOR]stream: [B][COLOR white]' + self.name.upper() + '[/B][/COLOR]',
                                          'File was removed from g2g.fm', 4000, self.icon)
