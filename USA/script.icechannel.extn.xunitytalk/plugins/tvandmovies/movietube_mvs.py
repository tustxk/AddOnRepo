...
class MovieTube(MovieSource):
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):                 
        new_url = 'http://www.movietube.so/index.php'
        import re
        ...
        headers={'Referer':'http://www.movietube.so/results.php', 'X-Requested-With':'XMLHttpRequest','Content-Type':'application/x-www-form-urlencoded; charset=UTF-8'}

        title = self.CleanTextForSearch(title) 
        name = self.CleanTextForSearch(name)

        form_data={ 'c':'result', 'a':'retrieve', 'p':'{"KeyWord":"%s (%s)","Page":"1","NextToken":""}' % (name,year) }
        content = net.http_POST('http://www.movietube.so/index.php', form_data, headers).content
