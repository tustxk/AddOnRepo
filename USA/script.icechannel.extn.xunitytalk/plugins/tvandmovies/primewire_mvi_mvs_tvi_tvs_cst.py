...
class PrimeWire(MovieIndexer, MovieSource, TVShowIndexer, TVShowSource, CustomSettings):
    ...
    img = 'https://raw.githubusercontent.com/Coolwavexunitytalk/images/93da52d618fa7c27b00eee25018f6a6d3fee7b04/primewire.png'
    ...
    def __init__(self):
        ...
        xml += '<setting id="custom_url" type="labelenum" label="URL" default="https://www.primewire.ag/" values="Custom|http://www.1channel.ch/|http://www.primewire.ag/|http://www.letmewatchthis.ch/|https://www.primewire.unblocked.pw/" />\n'
        xml += '<setting id="custom_text_url" type="text" label="     Custom" default="https://www.primewire.unblocked.pw/" enable="eq(-1,0)" />\n'
    ...
    def ExtractContentAndAddtoList(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''): 
        ...
        for item in re.finditer(r"<div class=\".+?\"><a href=\"(.+?)\" title=\"Watch (.+?)\"", content):
    ...
    def GetSection(self, indexer, section, url, type, list, page='', total_pages='', sort_by='', sort_order=''): 
        ...
        if section == 'main':
            ...
            self.AddSection(list, indexer, 'featured', 'Featured', custom_url+'?' + url_type + 'sort=featured', indexer)
            self.AddSection(list, indexer, 'views', 'Most Popular', custom_url+'?' + url_type + 'sort=views', indexer)
            self.AddSection(list, indexer, 'ratings', 'Highly Rated', custom_url+'?' + url_type + 'sort=ratings', indexer)
            self.AddSection(list, indexer, 'release', 'Date Released', custom_url+'?' + url_type + 'sort=release', indexer)
            self.AddSection(list, indexer, 'date', 'Date Added', custom_url+'?' + url_type + 'sort=date', indexer)
        elif section == 'genres':
            ...
            import re
            genres = re.search('<ul class="menu-genre-list">(.+?)</ul>', content)
            if genres:
            ...
            for letter in A2Z:
                self.AddSection(list, indexer, letter.lower(), letter, custom_url+'?' + url_type + 'letter=' + letter.lower(), indexer)                
        else:
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue): 
        ...
        if 'virginmedia.com/courtorders' in search_page_content or 'ukispcourtorders' in search_page_content or 'is not available through BSkyB' in search_page_content or 'ACCESS TO THE WEBSITES LISTED ON THIS PAGE HAS BEEN BLOCKED PURSUANT TO ORDERS OF THE HIGH COURT' in search_page_content:
            import xbmcaddon
            ADDON=xbmcaddon.Addon(id='script.icechannel.PrimeWire.settings')   
