...
class yify(MovieSource):
    ...
    def GetFileHosts(self, url, list, lock, message_queue,name): 
        ...
        pic=re.compile('pic=(.+?)&').findall (content)[0]
     
              
        self.AddFileHost(list, 'HD', url+'|'+pic,host='YIFY - '+name)
    ...
    def GetFileHostsForContent(self, title, name, year, season, episode, type, list, lock, message_queue):
        ...
        search_term = self.base_url+'?s='+name.lower().replace(' ','%20')
        ...
        match=re.compile(',"title":"(.+?)","link":"(.+?)".+?,"year":"(.+?)"').findall(html)
        ...
        for NAME , URL , YEAR in match:
            ...
            if name.lower() in NAME.lower():
                if year in YEAR:
                    ...
                    self.GetFileHosts(URL.replace('\\',''), list, lock, message_queue,NAME)
    ...
    def Resolve(self, url):
        ...
        html = net.http_POST('http://yify.tv/player/pk/pk/plugins/player_p2.php', data, headers).content
